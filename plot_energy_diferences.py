...
hycom = xray.open_dataset('results/netcdf_files/ww3_hycom.2016.nc')
...
usel = hycom.ucur.sel(longitude=lon, latitude=lat, method='nearest')
vsel = hycom.vcur.sel(longitude=lon, latitude=lat, method='nearest')
vel = np.sqrt(usel**2 + vsel**2)
hs = hycom.hs.sel(longitude=lon, latitude=lat, method='nearest')
...
for dt in range(dist, selef_nocur.time.shape[0]):
    ...
    plt.subplot(211)
    # import pdb; pdb.set_trace()
    plt.plot(hycom.time[dist::], vel[dist::])
    plt.plot(pd.to_datetime(selef.time[dt].values), vel[dt], 'or')
